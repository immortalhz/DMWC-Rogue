...
function Rogue.Settings()
    ...
    UI.AddDropdown("MH Poison", "", {"Disable","Wound","Instant"}, 1)
    UI.AddDropdown("OH Poison", "", {"Disable","Crippling","Numbing"}, 1)
    UI.AddHeader("")
    UI.AddToggle("Dont open from Stealth", nil, false)
    if DMW.Player.SpecID == "Outlaw" then
        -- UI.AddToggle("Pool for BTE", nil, false)
    ...
    if DMW.Player.SpecID == "Outlaw" then
        UI.AddToggle("AutoGouge", nil, false)
    end
